...
- **Existing smoke-test self-identification**: `smokeTest.ts` already tags its own web and API requests with an `x-kilotest-smoke: 1` header (`sendRequest`, used for every GET/POST path in `concretePaths`), specifically so that Kilotest can distinguish smoke-test traffic from real traffic if it ever needs to. Nothing currently reads that header. The two MCP probe requests, sent by the separate `sendMCPRequest` function, do not currently carry it (its own comment notes it deliberately omits headers, to exercise unmodified request handling), and this function is used both by the CI-gating smoke test (`.github/workflows/ci.yml`-triggered “Smoke Test” required check, which runs on every merge to `main`) and by the daily scheduled workflow (`.github/workflows/periodic-smoke-tests.yml`). Both invocations hit the live, deployed `kilotest.com` service, not a local or staging instance, so both would otherwise count as real usage.
...
- **One shared utility function, called at every counted site, not ad hoc calls per call site**: once metrics recording is being added to the codebase at all, every location that needs to record an event calls the same utility function, rather than each of the three call sites (web-page dispatch, MCP tool dispatch, API dispatch) growing its own inline increment-and-check logic. This keeps the smoke-test exclusion, the increment logic, and any future change to what gets recorded in one place, so a successor maintainer only needs to understand one function to understand how metrics recording works everywhere it is used, rather than auditing each call site separately for consistency.
- **Storage**: a single new file, `db/metrics.json`, following the same plain-JSON-file convention as `testRequests.json` and the comments files. It holds simple counters (see Data model below), incremented in memory as requests and MCP tool calls occur and periodically flushed to disk (not synchronously on every single request, to avoid a disk write per hit on a memory- and I/O-constrained host), plus written on graceful shutdown. This avoids both the unbounded growth of an append-only per-event log (which would need its own retention policy from day one) and the total data loss on every PM2 restart or deploy that a purely in-memory counter would suffer.
...
- **Excluding smoke-test traffic**: counting must not count `smokeTest.ts`’s own requests, since those run against the live production service on every merge to `main` (the required “Smoke Test” CI check) and once daily (the periodic workflow), and are synthetic, not real usage. The counting code checks for the `x-kilotest-smoke` header already sent by `sendRequest` and skips incrementing when it is present, rather than the metrics feature needing to invent its own exclusion signal. `sendMCPRequest` (the two MCP probe requests) is extended to send the same header, since nothing about its “exercise unmodified request handling” purpose depends on omitting this particular header, and MCP tool calls need the same exclusion as web/API calls do. Excluding by header, rather than by some other means such as an IP or user-agent allowlist, keeps the exclusion mechanism visible in one place (the smoke test’s own request code) and avoids hardcoding CI infrastructure details (runner IPs, etc.) into application code.
...
Counts exclude any request bearing the `x-kilotest-smoke` header (see Decisions above), so the numbers reflect real usage only, not CI-driven smoke-test traffic against the live service.
...
The steps below are sequential and, except for step 8, interdependent: for example, after step 1 alone `recordMetric` exists but no call site uses it yet, which the 100%-coverage requirement would fail, and after step 3 alone `web/metrics/` exists but is not reachable through `index.ts`’s dispatch, so nothing exercises it. Only step 8 (documentation) is a true no-op on functionality and could land as its own trailing commit without affecting `npm test`. Commit and push freely at intermediate points on the `observability` branch as work progresses, to avoid losing work, but only open the pull request to `main` once every step is complete and the full check suite (100% coverage, typecheck, lint, tests) passes, per Kilotest’s branch-protection rules (`docs/SERVICE.md`, “Branch protection”).

1. Add a small counters module (e.g. functions in `util.ts` or a new `metrics.ts`) that loads `db/metrics.json` at startup (creating it with zeroed counts if absent), exposes one shared utility function, e.g. `recordMetric(category, name, headers)`, that checks for `x-kilotest-smoke` in `headers` and no-ops when present, otherwise increments `category.name` in memory, and periodically flushes in-memory counts to disk on an interval plus on process shutdown. This one function is the sole place that implements the smoke-test exclusion and the increment logic.
2. Call `recordMetric` from the three call sites identified above: the generic `.html` GET dispatch in `index.ts` (page views), `mcp.ts`’s tool-call handling (one call per tool, at the point each tool’s handler is invoked), and the `api/*.ts` dispatch (operation calls). Each site passes its own category and name but otherwise calls the same function the same way; no call site implements its own increment or exclusion logic.
...
4. Wire `metrics.html` into `index.ts`’s `answer{}` table and `routes.GET`, and add the path to the Caddyfile allowlist (repository copy and `docs/SERVICE.md`’s copy) and to `smokeTest.ts`’s `concretePaths.GET`, per the existing “add a new route” checklist in `docs/SERVICE.md`. Since `/metrics.html` itself will be hit by the smoke test like any other GET path, and a GET is not itself a countable event under this plan’s data model (only page views, tool calls, and API operations are counted, and `/metrics.html` is a page view), verify during implementation whether the smoke test’s own visit to `/metrics.html` needs the same `x-kilotest-smoke` exclusion as everything else; it does, for consistency, even though its self-referential nature makes the miscount low-stakes.
5. Add the `x-kilotest-smoke: 1` header to `sendMCPRequest` in `smokeTest.ts` (currently sent only by `sendRequest`), so the MCP `initialize` and browser-style GET probes are excluded from the MCP tool-call counts the same way web/API smoke requests are excluded from page-view and operation counts.
6. Implement the `authCode` gate in `web/metrics/index.ts` per the Access decision above: return `{status: 'error', message: 'Invalid authorization code'}` when the submitted `authCode` query-string parameter does not equal `process.env.AUTH_CODE`, matching the existing pattern in `web/reannotateForm/index.ts`. Note the gate’s location (one check, in one file) for a future change to make the page public, so that revisiting this decision later is a small, easily found edit.
7. Tests: unit-test `recordMetric` directly (increment, flush, load-on-missing-file, and the `x-kilotest-smoke` exclusion), and separately confirm each of the three call sites actually calls it (so a future refactor that bypasses the shared function is caught), plus test `web/metrics/`’s `answer()`, and extend `index.test.ts` to cover the new route, maintaining the required 100% coverage.
8. Document the new page, its data model, and the smoke-test exclusion briefly in `docs/SERVICE.md`, alongside the existing “Alerting” and “Health monitoring” sections, so this becomes part of the same operational-documentation set.

### Observability verification

- `npm run typecheck`, `npm run lint`, and `npm test` pass with 100% coverage maintained.
- Locally, exercise a few web pages, MCP tool calls, and API operations, then load `/metrics.html` and confirm the counts increment as expected and persist across a process restart (i.e., the flush-to-disk and load-on-startup behavior both work).
- `node smokeTest.ts` confirms `/metrics.html` is both allowlisted and reachable, and, run against a local instance, confirms the counts in `db/metrics.json` do not change as a result of the smoke test’s own requests (web, API, and MCP), verifying the `x-kilotest-smoke` exclusion works end to end.
