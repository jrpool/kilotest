...
test('hideReportForm displays a list of reports on a GET request', async () => {
...
test('hideReportForm ignores a report query string on a GET request', async () => {
  ...
  const result: any = await answer(null, 'authCode=test-auth-code&report=260101T0009-brd', 'GET');
  assert.equal(result.status, 'ok');
  // The report should NOT have been hidden by a mere GET request.
...
test('hideReportForm hides a report with valid auth code on a POST request', async () => {
  ...
  const result: any = await answer(null, 'authCode=test-auth-code&report=260101T0009-brd', 'POST');
  assert.equal(result.status, 'ok');
  // The report should now be in the hidden directory.
  await fs.access(hiddenPath);
});

test('hideReportForm returns an error when hiding a nonexistent report on a POST request', async () => {
