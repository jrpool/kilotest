...
test('handleComment never rejects for a full inbox when TUTORIAL_WEB_COMMENTS_MAX is 0 (no limit)', {timeout: 500}, async () => {
  process.env.TUTORIAL_WEB_COMMENTS_MAX = '0';
  try {
    ...
    delete process.env.TUTORIAL_WEB_COMMENTS_MAX;
    await fs.writeFile(getCommentsPath(), '[]\n');
  }
});
